...
const { Transaction, TxIn,
  UnspentTxOut, validateTransaction } = require('./transaction');
...
const isValidTxForPool = (tx, aTtransactionPool) => {
  ...
  const containsTxIn = (txIns, txIn) => {
    return txIns.find(txPoolIn => txPoolIn.txOutId === txIn.txOutId && txPoolIn.txOutIndex === txIn.txOutIndex);
  };
